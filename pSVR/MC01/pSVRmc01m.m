...
for i_r = 1:numel(s.analysis.ROI)
    ...
    out_dir = fullfile('MC01res','psvr_resultsm',s.results.folder_name,[s.analysis.ROI{i_r}]);
    if ~exist(out_dir,'dir'), mkdir(out_dir); end

    %% Get labels and reformat data based on trial info
    [s, data] = get_datam(i_SUB,1,s,dat);
    ...
    saveFile = matfile(fullfile(out_dir, [subcode 'pSVRresultsm.mat']), 'Writable',true);
    saveFile.D = D;
    saveFile.s = s;
