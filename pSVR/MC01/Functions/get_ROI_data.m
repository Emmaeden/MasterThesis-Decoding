...
function [data,regs] = get_ROI_data(i_SUB,s)
    ...
    DIR.baseDir = fullfile('/disco/emma','matlabscripts','MC01','data');
    DIR.lvl1 = fullfile('/disco/thomas/MC01/data', sprintf('sub_%02d',i_SUB),'lvl1','orientation_activation'); %% change
    DIR.dataDir = fullfile(DIR.baseDir,sprintf('sub%02d',i_SUB)); 
    mkdir(fullfile(DIR.dataDir,'results'));
    DIR.resDir  = fullfile(DIR.dataDir, 'results'); 
    DIR.runDir = fullfile(DIR.baseDir,sprintf('sub%02d',i_SUB),'run_00');
    
    % get spm activity masks
    activity_dir = fullfile(DIR.dataDir,'model'); % link the model for each subject to get the beta maps
    ...
    for i_roi = 1:nROI
        ...
        spm_mask = spm_vol(fullfile(activity_dir,'mask.nii')); % link the masks from the model folder 
        mask_name = [s.analysis.ROI{i_roi},'_Mask.nii'];
        roi_mask = spm_vol(fullfile('/disco/thomas/MC01/data',sprintf('sub_%02d',i_SUB) ,'rois',mask_name));
        ...
        [roi_idx, ROI_nii] = roi_extract_activity(i_SUB, DIR, mask,n_voxel); % find this function in vivien folder
        mask = ROI_nii;
   ...
   % for i_session = 1:4
     for i_run = 1:8  
            ...
            vols = dir(fullfile([DIR.runDir(1:end-1), num2str(i_run)], 'rew*')); 
