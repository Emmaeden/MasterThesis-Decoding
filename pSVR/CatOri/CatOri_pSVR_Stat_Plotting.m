...
resPath = '/disco/emma/CatOriRes/psvr_results/res_pSVR_CatOri/wFEF_Mask';
addpath(resPath);
files = dir(fullfile(resPath, '*.mat'));
res = NaN(39,26,24,8); %36 subjects, 16 trials, 8 runs

% Load and reshape data
for ii = 1:numel(files)
    ld = load(fullfile(resPath, files(ii).name));
    for fir=1:26
        reshapedData = reshape(ld.D.angle_diff{fir}, [24 8]); % (subject x trials x runs)
...
chance_level =90;
% Across sessions
early_runs = mean(dres(:,:,:,1:4), [3,4]) ; % subject X time
late_runs = mean(dres(:,:,:,5:8), [3,4]) ;
% Within sessions
% early_runs = mean(dres(:, :,:, [1 2 5 6]), [3,4]) ; % subject X time
% late_runs = mean(dres(:, :,:, [3 4 7 8]), [3,4]) ;

SemEarly = std(early_runs,1)/sqrt(39);
SemLate =  std(late_runs,1)/sqrt(39);
...
earlyPerc = chance_level - mean(early_runs(:,8:13),2);
latePerc = chance_level - mean(late_runs(:,8:13),2);
earlyWm = chance_level - mean(early_runs(:,14:25),2);
lateWm = chance_level - mean(late_runs(:,14:25),2);

data = [earlyPerc,latePerc,earlyWm,lateWm]; % 23x4
means = mean(data);
sems = std(data)./sqrt(36);
