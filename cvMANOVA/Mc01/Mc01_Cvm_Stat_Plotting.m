...
meanEarly = squeeze(mean(DsEarly(:,:,2),1));
meanLate =  squeeze(mean(DsLate(:,:,2),1));
...
h1 =plot(T,mean(DsEarly(:,:,2),1),'LineWidth',2, 'Color',color1);
h2 =plot(T,mean(DsLate(:,:,2),1),'LineWidth',2,'Color',color2);
...
title('Average Pattern Distinctness over Time - IPS', 'FontSize', 14, 'FontWeight', 'bold');
...
xticklabels({'Visual Cortex','IPS','FEF'});
...
title('Pattern Distinctness Across Sessions per ROI');
set(gca, 'FontSize', 12);
