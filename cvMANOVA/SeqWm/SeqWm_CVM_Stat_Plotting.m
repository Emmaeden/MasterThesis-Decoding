...
load WithinNewResults.mat %SeqWm_AcrossSession_ResultsNew.mat 
DsEarly = cat(3, ResultsEarly{:});
DsEarly = reshape([DsEarly{:}], size(DsEarly));
DsEarly = permute(DsEarly, [3, 1, 2]);

%load SeqWm_Results_LateSessions.mat
DsLate = cat(3, ResultsLate{:});
DsLate = reshape([DsLate{:}], size(DsLate));
DsLate = permute(DsLate, [3, 1, 2]);

%% ttest across Time
MeanSubEarly = mean(DsEarly(:,:,3),2); % stim onset and whole delay period 1s-18s
MeanSubLate = mean(DsLate(:,:,3),2);
[hsub ,psub, stat] = ttest(MeanSubEarly-MeanSubLate);
fprintf('t-test results Within Time : h = %d, p = %.5f\n',hsub,psub);

%%
percDelay = 1:10; % 1s-10s Perception  
WmDelay = 11:23; % 9s-18s Working Memory 

%% ttest Across Delays
[he pe state] = ttest(mean(DsEarly(:,percDelay,1),2)-mean(DsLate(:,percDelay,1),2));
fprintf('t-test Perception Delay Across Subjects: h = %d, p = %.5f\n',he,pe);
[hl pl statl] = ttest(mean(DsEarly(:,WmDelay,1),2)-mean(DsLate(:,WmDelay,1),2));
fprintf('t-test Working Memory Delay Across Subjects: h = %d, p = %.5f\n',hl,pl);

%% Sem
SemEarly = std(DsEarly(:,:,1),[],1)/sqrt(23);
SemLate=  std(DsLate(:,:,1),[],1)/sqrt(23);
...
meanEarly = squeeze(mean(DsEarly(:,:,1),1));
meanLate =  squeeze(mean(DsLate(:,:,1),1));
...
h1 =plot(T,mean(DsEarly(:,:,1),1),'LineWidth',2, 'Color',color1);
h2 =plot(T,mean(DsLate(:,:,1),1),'LineWidth',2,'Color',color2);

%Add shaded background for specific intervals 
% 
% fill([EarlyT_sec fliplr(EarlyT_sec)], [repmat(-0.01,1,2) repmat(0.25,1,2)],...
%     [0.7 0.7 0.9], 'FaceAlpha',0.15,'EdgeColor','none');
% % shaded region for late delay
% fill([LateT_sec fliplr(LateT_sec)], [repmat(-0.01,1,2) repmat(0.25,1,2)],...
...
title('Early Learning phase vs Later learning phase Within Sessions ')
...
ylabel('Pattern Distinctness ');
title('Pattern Distinctness Within Sessions per ROI');
set(gca, 'FontSize', 12);
